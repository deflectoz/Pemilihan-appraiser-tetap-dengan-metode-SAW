...
class Rangking extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->page->setTitle('SPK SAW | Rangking');
        $this->page->setTitleContent('Halaman Hasil Perangkingan');
        $this->page->setLoadJs('assets/js/print_rank');
    ...
    public function laporan_pdf(){
         ...
         */
        $table1 = $this->initialTableSAW($calon);
        ...
        $this->pdf->filename = "laporan-saw.pdf";
        $this->pdf->load_view('laporan_pdf', $tableFinal);
    ...
    private function initialTableSAW($calon)
    {
        $nilai = $this->MNilai->getNilaiCalon();

        $dataInput = array();
        $no = 0;
        foreach ($calon as $item => $itemcalon) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemcalon->kdCalon == $itemNilai->kdCalon) {
                    $dataInput[$no]['calon'] = $itemcalon->calon;
 ...
 ///
    ...
    {
        $this->db->order_by('Rangking', 'DESC');
        $sawData = $this->MSAW->getSortTotalByDesc();
        $sawDataAsArray = json_decode(json_encode($sawData), true);
        foreach ($sawDataAsArray as $key => $value) {
           $totVal[$key]=$value['Total'];
           $valCalon[$key]=$value['calon'];
        }
        $c = array_merge($totVal, $valCalon);
        $rankings = array();
        rsort($totVal);
        $rank = 1;
        $tie_rank = 0;
        $prev_score = -1;
        foreach ($totVal as $name => $score) {
        ...
        foreach ($rankings as $item => $value) {
            $dataUpdate = array(
                'Rangking' => $value['rank']
            );
            $where = array(
